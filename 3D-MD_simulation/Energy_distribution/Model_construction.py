...
def main(ell_number, major_axis, minor_axis, wave_length, P_ave, t, f, outputpath ,d , n, R):
    # Set the shell at a specific distance
    step_interval = minor_axis/ell_number  # Suppose a layer of spherical shell is set at 0.25 μm each time
    ...
    lambda_ = wave_length  # Laser wavelength
    n = n  # Refractive index of 4H-SiC
    R = R  # Reflection coefficient of 4H-SiC
    d = d  # Sample thickness
    k = calculate_k(R, n)
    A = absorption_energy(lambda_, R, d, k)  # The absorbance of energy

    w0 = 2
    layer_n = repeat_
    P_ave = P_ave   # The amount of energy that corresponds to the reduction of the radius
    t = t  # Laser action time 285 fs
    ...
    x_layer_In = np.linalg.solve(S_area_, In_TTM_aft)*6.241509e18/(1.25e8)

    print('x_layer_In: \n', x_layer_In)
    with open(f'{outputpath}' + ".txt", 'w') as file:
